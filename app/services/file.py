...
def update_upload_session(
    db: Session, session_id: int, processed_files: int
) -> UploadSession:
    """
    ...
    app_logger.info(
        f"Updated upload session: {db_session.id}, processed: {processed_files}"
    )
    return db_session


def create_processing_task(
    db: Session, image_id: int, task_type: str, params: Dict[str, Any]
) -> ProcessingTask:
    """
    Tạo task xử lý ảnh
    """
