    ...
    def resize_image(
        ...
        try:
            ...
            with PILImage.open(image_path) as img:
                ...
                if output_path is None:
                    directory = os.path.dirname(image_path)
                    filename = f"resized_{size[0]}x{size[1]}_{os.path.basename(image_path)}"
                    output_path = os.path.join(directory, filename)
        ...
        try:
            ...
            with PILImage.open(image_path) as img:
                ...
                if filter_type == "grayscale":
                    ...
                    app_logger.warning(f"Filter {filter_type} not implemented with PIL, using grayscale instead")
                    filtered_img = img.convert('L')
...
def process_image(db: Session, task_id: int) -> bool:
    ...
    try:
        ...
        if task.task_type == "resize":
            ...
            result_path = image_processor.crop_image(image.file_path, (left, upper, right, lower))
            ...
            result_path = image_processor.create_thumbnail(image.file_path, (width, height))
        ...
        # Kiểm tra kết quả
        if not result_path:
            app_logger.error(f"Failed to process image: {image.id}, task: {task.task_type}")
...
def create_thumbnails(db: Session, image_id: int) -> List[Thumbnail]:
    ...
    for size_info in thumbnail_sizes:
        try:
            ...
            app_logger.info(f"Created thumbnail: {thumbnail.id}, size: {size_info['size']}")
